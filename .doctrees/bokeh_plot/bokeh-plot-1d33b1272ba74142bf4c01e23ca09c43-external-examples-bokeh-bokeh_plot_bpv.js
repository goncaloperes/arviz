(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("d9c5a474-cbc9-4d14-acfe-4b5a2066a6c5");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'd9c5a474-cbc9-4d14-acfe-4b5a2066a6c5' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js": "T2yuo9Oe71Cz/I4X9Ac5+gpEa5a8PpJCDlqKYO0CfAuEszu1JrXLl8YugMqYe3sM", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js": "98GDGJ0kOMCUMUePhksaQ/GYgB3+NH9h996V88sh3aOiUNX3N+fLXAtry6xctSZ6", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js": "89bArO+nlbP3sgakeHjCo1JYxYR5wufVgA3IbUvDY+K7w4zyxJqssu7wVnfeKCq8"};
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.2.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.2.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"e30ea541-d2e6-4876-982c-f1fdffdc734b":{"roots":{"references":[{"attributes":{"axis":{"id":"65319"},"dimension":1,"ticker":null},"id":"65322","type":"Grid"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"65367"},"selection_policy":{"id":"65368"}},"id":"65348","type":"ColumnDataSource"},{"attributes":{},"id":"65370","type":"UnionRenderers"},{"attributes":{"children":[[{"id":"65306"},0,0]]},"id":"65374","type":"GridBox"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65343","type":"Line"},{"attributes":{},"id":"65313","type":"LinearScale"},{"attributes":{},"id":"65307","type":"DataRange1d"},{"attributes":{"bottom":1.4596776592684024,"fill_alpha":0.1,"fill_color":"#2a2eec","top":0.6850827603275129},"id":"65347","type":"BoxAnnotation"},{"attributes":{"formatter":{"id":"65364"},"ticker":{"id":"65320"}},"id":"65319","type":"LinearAxis"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","top_units":"screen"},"id":"65331","type":"BoxAnnotation"},{"attributes":{"source":{"id":"65342"}},"id":"65346","type":"CDSView"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"65323"},{"id":"65324"},{"id":"65325"},{"id":"65326"},{"id":"65327"},{"id":"65328"},{"id":"65329"},{"id":"65330"}]},"id":"65333","type":"Toolbar"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65355","type":"Line"},{"attributes":{"below":[{"id":"65315"}],"center":[{"id":"65318"},{"id":"65322"},{"id":"65347"}],"left":[{"id":"65319"}],"output_backend":"webgl","plot_height":500,"plot_width":500,"renderers":[{"id":"65345"},{"id":"65351"},{"id":"65356"}],"title":{"id":"65358"},"toolbar":{"id":"65333"},"toolbar_location":null,"x_range":{"id":"65307"},"x_scale":{"id":"65311"},"y_range":{"id":"65309"},"y_scale":{"id":"65313"}},"id":"65306","subtype":"Figure","type":"Plot"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"65344","type":"Line"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"65350","type":"Line"},{"attributes":{},"id":"65367","type":"Selection"},{"attributes":{"source":{"id":"65353"}},"id":"65357","type":"CDSView"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"65365"},"selection_policy":{"id":"65366"}},"id":"65342","type":"ColumnDataSource"},{"attributes":{"data_source":{"id":"65353"},"glyph":{"id":"65354"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65355"},"selection_glyph":null,"view":{"id":"65357"}},"id":"65356","type":"GlyphRenderer"},{"attributes":{},"id":"65365","type":"Selection"},{"attributes":{"overlay":{"id":"65332"}},"id":"65327","type":"LassoSelectTool"},{"attributes":{},"id":"65329","type":"SaveTool"},{"attributes":{},"id":"65364","type":"BasicTickFormatter"},{"attributes":{"data_source":{"id":"65342"},"glyph":{"id":"65343"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65344"},"selection_glyph":null,"view":{"id":"65346"}},"id":"65345","type":"GlyphRenderer"},{"attributes":{},"id":"65362","type":"BasicTickFormatter"},{"attributes":{},"id":"65326","type":"WheelZoomTool"},{"attributes":{"axis":{"id":"65315"},"ticker":null},"id":"65318","type":"Grid"},{"attributes":{},"id":"65369","type":"Selection"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"xs_units":"screen","ys_units":"screen"},"id":"65332","type":"PolyAnnotation"},{"attributes":{"source":{"id":"65348"}},"id":"65352","type":"CDSView"},{"attributes":{},"id":"65328","type":"UndoTool"},{"attributes":{"toolbar":{"id":"65375"},"toolbar_location":"above"},"id":"65376","type":"ToolbarBox"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"65349","type":"Line"},{"attributes":{},"id":"65309","type":"DataRange1d"},{"attributes":{},"id":"65324","type":"PanTool"},{"attributes":{},"id":"65316","type":"BasicTicker"},{"attributes":{"formatter":{"id":"65362"},"ticker":{"id":"65316"}},"id":"65315","type":"LinearAxis"},{"attributes":{},"id":"65366","type":"UnionRenderers"},{"attributes":{"text":"y","text_font_size":{"value":"15pt"}},"id":"65358","type":"Title"},{"attributes":{},"id":"65323","type":"ResetTool"},{"attributes":{"data_source":{"id":"65348"},"glyph":{"id":"65349"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"65350"},"selection_glyph":null,"view":{"id":"65352"}},"id":"65351","type":"GlyphRenderer"},{"attributes":{"toolbars":[{"id":"65333"}],"tools":[{"id":"65323"},{"id":"65324"},{"id":"65325"},{"id":"65326"},{"id":"65327"},{"id":"65328"},{"id":"65329"},{"id":"65330"}]},"id":"65375","type":"ProxyToolbar"},{"attributes":{},"id":"65320","type":"BasicTicker"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"65354","type":"Line"},{"attributes":{"callback":null},"id":"65330","type":"HoverTool"},{"attributes":{},"id":"65368","type":"UnionRenderers"},{"attributes":{"data":{},"selected":{"id":"65369"},"selection_policy":{"id":"65370"}},"id":"65353","type":"ColumnDataSource"},{"attributes":{"children":[{"id":"65376"},{"id":"65374"}]},"id":"65377","type":"Column"},{"attributes":{},"id":"65311","type":"LinearScale"},{"attributes":{"overlay":{"id":"65331"}},"id":"65325","type":"BoxZoomTool"}],"root_ids":["65377"]},"title":"Bokeh Application","version":"2.2.3"}}';
                  var render_items = [{"docid":"e30ea541-d2e6-4876-982c-f1fdffdc734b","root_ids":["65377"],"roots":{"65377":"d9c5a474-cbc9-4d14-acfe-4b5a2066a6c5"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();